...
import { Plus, Pencil, Copy, Trash2, Link2, ToggleLeft, ToggleRight } from "lucide-react"
...
export function FormList({ forms, onEdit, onCreate, onRefresh }: Props) {
    ...
    const handleDelete = async (form: LeadForm) => {
        ...
        const res = await deleteLeadForm(form.id)
                ...
                {forms.map(form => (
                    <div key={form.id} className="flex items-center gap-3 p-3 border rounded-lg group hover:bg-muted/30 transition-colors">
                        ...
                        <div
                            ...
                            style={{ backgroundColor: form.style?.accentColor || "#2563eb" }}
                        />
                        ...
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleToggle(form)} title={form.status === "active" ? "Deactivate" : "Activate"}>
                                {form.status === "active" ? <ToggleRight className="h-3.5 w-3.5" /> : <ToggleLeft className="h-3.5 w-3.5" />}
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDuplicate(form)} title="Duplicate">
                                <Copy className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDelete(form)} title="Delete">
